...
基于 **Node.js + Vite + React + TypeScript + React Router** 的静态前端，在 **Windows、macOS、Linux** 上均可运行；本仓库日常在 **Linux** 开发与调试，**Windows** 侧文档按「给最终用户使用」编写。
...
- **编辑 / 删除**：列表卡片上「编辑」进入 `/edit/:id`；「删除」移除该条，均写回 localStorage。
- **备份与迁移**：列表页「数据备份」区可 **导出 JSON**（与 localStorage 数组结构一致，空列表可导出为 `[]`）；**导入 JSON** 按 **`id` 合并**（同 id 以文件为准覆盖、文件中的新 id 追加、仅本地有的 id 保留）。选择文件并校验通过后需 **确认**，确认框中会提示预计覆盖/新增/保留条数，便于多人分文件维护后再合并。
...
产品列表保存在浏览器 **localStorage** 中，键名为 **`web_pick_products`**（定义见 `src/persistence/productsStorage.ts`），与操作系统无关。

**导出**：在列表页「数据备份」中点击「导出 JSON」，得到 `tier-slist-backup-YYYY-MM-DD.json`（见 `src/lib/backupFilename.ts`）。文件内容为 **JSON 数组**，元素为当前版本的产品对象（含 `id`、区域、品类、三档 `economy` / `mid` / `high`、`selectedTier` 等）。空列表时也可导出，文件为 `[]`。

**导入**：选择上述格式的 JSON 文件。解析失败或任一条记录无法识别时 **不会修改** 本地数据，并显示错误说明。校验通过后需 **确认** 再写入，规则如下。

| 情况 | 行为 |
|------|------|
| 文件中的 `id` 与本地某条相同 | 以文件中的整条记录 **覆盖** 本地该条 |
| 文件中的 `id` 本地不存在 | **追加** 为新记录 |
| 本地有、文件中未出现的 `id` | **保留** 不变 |
| 文件中同一 `id` 出现多次 | **以后出现的为准** |

若需要列表与文件 **完全一致**（等价于旧版「整表替换」），可先清空本地数据（例如导出备份后，在界面中删除全部产品）再导入同一份文件。请勿手动篡改 JSON 结构；若文件含旧版扁平字段，加载逻辑会尽量迁移，但建议以本应用导出的文件为准。
